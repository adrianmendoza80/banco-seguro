...
app.post('/verificar', async (req, res) => {
  ...
  try {
    ...
    if (!tokenCapturado) {
      return res.json({ autorizado: false, confianza: 0, mensaje: 'No se detectó un rostro. Mirá directo a la cámara.' });
    }
    ...
    if (mejorMatch && mayorConfianza >= UMBRAL) {
      ...
      return res.json({ autorizado: false, confianza: Math.round(mayorConfianza), mensaje: 'Rostro no reconocido' });
    }
  } catch (e) {
